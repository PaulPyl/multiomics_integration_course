...
result.diablo.tcga <- block.plsda(X = theX, Y = theY$IGHV) # run the method
...
```{r}
mRNA_loadings <- result.diablo.tcga$loadings$mRNA |> as_tibble(rownames = "EnsemblGeneID") |> mutate(abs1 = abs(comp1), abs2 = abs(comp2), absRank1 = rank(-abs1), absRank2 = rank(-abs2))
```
...
```{r}
mutations_loadings <- result.diablo.tcga$loadings$Mutations |> as_tibble(rownames = "GeneName") |> mutate(abs1 = abs(comp1), abs2 = abs(comp2), absRank1 = rank(-abs1), absRank2 = rank(-abs2))
mutations_loadings |> dplyr::filter(absRank1 <= 10 | absRank2 <= 10) |> arrange(absRank1, absRank2)
```
